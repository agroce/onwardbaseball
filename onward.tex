...
in light of the critical differences, to guide future work on software
engineering metrics.
  \end{abstract}
...
and government mainframes. The deeper connection, however, which this essay addresses, is that baseball
...
statistics and started making use of more sophisticated, nuanced, statistics
...
(to stay polite, this is a family paper!) “BS” involved) and
...
baseball and software engineering would require considerable argument
and evidence.   But
...
with statement coverage mentioned above.
...
\begin{itemize}
  \item Baseball and software engineering are both, when played for
    ...
    \item The ``game''  is deeply structured and appeals to
      ...
      \item Arguably, for some historical, contingent reason, Americans are best
        at it, though Brits invented it.  The Japanese can certainly
        play, though maybe only in video games, but that's one of the
        top money-making divisions, with a huge audience.  
 ...
 None of these things contradict a narrative where ``doing what OBP
 ...
 \begin{itemize}
   \item Baseball has a profoundly binary structure:  games are won or
     ...
     (perhaps with degrees of win for the batter) for the pitcher or
     the batter; an inning has a score that is a set of of evaluations
    ...
    \item Baseball's outcomes are almost entirely \emph{evaluated
      ...
      negative payoffs clear for years, in some cases for decades.
 ...
 the bug is obscure and unlikely enough.  If a guy in the outfield
 ...
 people in the management structure of his team, and Beane says the
 others are trying to replace a certain critical individual player, but this is a
 mistake.  He adds up the on-base-percentages of three players, and
 ...
 you know the values to use (and using highly-relevant
 on-base-percentage vs. much more famous, much  more lossy, batting average, matters here).
...
the structure of the game means we need to give up, at least for now.
...
technical debt or cognitive load. And often, one swing is better than
...
The way to play is to give up on the game you can't win right now.  In
...
desirable statistical tools (developer evaluation tools for hiring,
...
\begin{itemize}
  ...
  \item Dependency and API stability:  when a library API breaks
    ...
    \item Code review turnaround (are bugs detected in code that
      ...
      developers but are poor reviewers, even
    \item Incident/outage attribution
 ...
 Who is Bill James?  In 2006, Time magazine named him one of the 100
